...
4. 对非小任务写任务合同，尤其要填 `隔离审查`：`不开 / 需要 / 延后`，以及触发原因、审查轴和结果回灌位置。
5. 只有执行阶段 1 技术步骤时，再读 `src/phase1/pipeline.md` 和相关 references。

高风险任务不要等用户提醒才开 subagent。遇到 `S4` 可视化、`D03/D04`、真实 `00-04` 成品、长上下文后的重构、用户已经指出质量不可信的文件，视为项目已给出默认授权：已有冻结稿，或已有明确可并行侧线，就开小白读者、证据审查、图文审查、探索型或验证型 subagent；只有两者都没有时，才写清延后审查点。Subagent 只接收冻结稿、读者身份、预期出口和禁止事项，不接收完整聊天记录。若决定不开，必须写明原因和残余风险。
