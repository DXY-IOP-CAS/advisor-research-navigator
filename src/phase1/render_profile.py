...
功能：
  ...
  2. 从 career_stages.json 或 --stages 读取学术阶段配置
  3. 内联过滤：
     - OA 独有论文：通过合著者+期刊+机构网络评分（< 1 分过滤）
     - arXiv 独有论文：无 DOI 则过滤（无法交叉验证）
  4. 每篇论文附带 DOI/arXiv 超链接
  5. 自动生成 §1 身份标识、§2 学术履历表、§4 论文分组表格、§8 数据质量统计
  6. 支持 --department 参数定制内容
...
def generate(data: dict, output_path: str, stage_config: list = None,
    ...
    L(f'| Google Scholar ID | {prof.get("gs_id", "")} |')
    L(f'| OpenAlex ID | {prof.get("oa_id", "")} |')
