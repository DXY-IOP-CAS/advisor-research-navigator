...
输出 `mode: standard` 时继续渲染。输出 `mode: conservative_required` 时，不要凭感觉多轮乱搜；必须同时阅读 `reason` 和 `next_actions`，按行动清单补官网身份、剔除或标注单源论文风险，再重新运行 risk gate。身份字段修正走 `apply_identity_review.py`，论文剔除走 `apply_paper_review.py`；详细命令以 `.claude/skills/research-advisor/references/phase1-core.md` 为执行单一事实源。
---
