...
新导师或新一轮 active state 先运行 `phase1_init.py`。它负责创建标准目录、写 `_internal/latest.txt`、写 `_internal/seed.json`，并在 stdout 只打印后续步骤使用的 `prof_dir`。archive 路径只由 `ProfDirResolver` 根据 `_internal/latest.txt` 自动解析，不给执行者手动使用。AI 不要手动 `mkdir`，也不要手动拼 `_internal/archive/<ts>`。
...
保存为 `career_stages.json`，与 `verified_ids.json` 一起存入 `_internal/archive/<timestamp>/`。
...
Phase B/C 脚本统一优先使用 `--prof-dir` 参数，由 `ProfDirResolver` 从 `_internal/latest.txt` 自动推导当前 archive 目录。常规执行不要让 AI 手动拼 `_internal/archive/<ts>` 路径。
...
#   career_stages.json
python src/phase1/validate_verified_ids.py --prof-dir "$PROF"
python src/phase1/validate_career_stages.py --prof-dir "$PROF"
...
**ProfDirResolver**（`utils.py`）：脚本内部用 `ProfDirResolver(prof_dir)` 自动解析所有路径，从 `_internal/latest.txt` 读取时间戳、拼接 `_internal/archive/<ts>/`、定位各文件。导师根目录不得再放 `latest.txt`，也不会作为 active state 读取来源。
...
默认不要向用户索要或临时填写邮箱。若执行环境已经预配置维护者邮箱，可在 `step3_openalex.py` 中加 `--email <configured-email>` 进入 10 req/s polite pool；没有配置时脚本按无 email 路径限速到约 1 req/s，这是可接受降级，不改变 `姓名 + 机构路径 + 官网 URL` 的最小输入契约。
...
输出 `mode: standard` 时继续渲染。输出 `mode: conservative_required` 时，不要凭感觉多轮乱搜；必须同时阅读 `reason` 和 `next_actions`，按行动清单补官网身份、剔除或标注单源论文风险，再重新运行 risk gate。身份字段修正走 `apply_identity_review.py`，论文剔除走 `apply_paper_review.py`；详细命令以 `.agents/skills/research-advisor/references/phase1-core.md` 为执行单一事实源。
---
...
```bash
python src/phase1/verify_profile.py --prof-dir "$PROF"
```
