...
**渲染后 AI 润色**：脚本生成论文表格和运行统计。AI 在脚本输出基础上补充以内容。**目标是让学生读完后对这个老师的研究方向、技术路线、职业发展到当前状态有清晰的认识。**

一个"成品"画像应该回答以下问题：

1. **研究方向总览**（1 段，不是关键词列表）：
   - 这个组主要解决什么问题？实验还是理论？用什么技术手段？
   - 不是列关键词，而是串成一段话，让人读完就能理解这个组在做什么。

2. **学术履历 + 研究脉络**（这是画像的核心价值）：
   - 从博士到现在的职业路径。每个阶段的**变化**比"发了什么论文"更重要。
   - 每条履历写清楚：什么时间、什么机构、什么职位。
   - 每个阶段的论文表格前有一段话说明：**这个阶段他在研究什么问题？跟上一阶段相比有什么转型？为什么转？** 转型往往是因为换了机构换了方向。
   - 示例（不是模板，是质量标准）：
     ```
     ### 4.1 博士及博后早期（~2013，近代物理所）
     研究主题：离子-原子碰撞动力学。
     这个阶段张鹏举在近代物理所研究离子与惰性气体原子的碰撞电离过程，采用(e,2e)符合测量技术。论文集中在电子发射谱、多电离过程等方向。
     ```
   - 禁止：只写"论文数 N 篇"然后列表格，没有上下文。

3. **当前研究方向详解**（what they're doing NOW）：
   - AI 从官网的"研究方向"文本、最新论文标题、新闻中提取。
   - 写 3-4 个研究方向，每个含 2-3 句中英文描述。
