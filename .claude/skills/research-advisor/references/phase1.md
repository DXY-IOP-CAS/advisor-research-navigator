...
**核心思路**：AI 负责叙事（会毁表格），脚本负责表格（完整准确）。两步各取所长，最后合并。
...
草稿含全部论文表格（标题超链接）、运行统计、数据质量说明。没有叙事。

#### 7.2 AI 润色（输出完整画像）

AI 基于草稿，用 `Write` 工具写出完整画像到根目录。这一步叙事会完整，但论文表格可能被 AI 压缩成"代表性论文"。

```bash
output/<机构>/<部门>/<姓名>/01_基础画像.md    # AI 润色版（叙事完整，表格可能不全）
```

AI 需补充：§2 履历、§3 方向、§6 合作网络、§7 公开信息、§9 来源。每阶段加叙事段落。

**排序规则**：§2 学术履历和 §4 论文表格必须统一使用**从旧到新**（chronological）排序。不写倒序。

#### 7.3 合并论文表格（脚本修复表格）

AI 润色版的叙事保留，论文表格替换回草稿中的完整表格。

```bash
python src/phase1/merge_tables.py \
  output/<机构>/<部门>/<姓名>/01_基础画像.md \
  output/<机构>/<部门>/<姓名>/archive/<timestamp>/01_基础画像_draft.md \
  -o output/<机构>/<部门>/<姓名>/01_基础画像.md
```

merge_tables.py 按 `### 4.1` / `4.2` 等序号匹配阶段，对每个阶段保留 AI 的叙事、替换为草稿的完整表格。

#### 7.4 验证成品

- [ ] §4 论文表格行数是否等于 merged.json 中论文数（如有过滤需说明）
- [ ] 有 DOI/arXiv 的论文标题是否含超链接
- [ ] §2 履历、§3 方向、§6 合作、§7 公开信息、§9 来源存在
- [ ] 名字格式：中文 (English)
- [ ] 无导师评价内容
