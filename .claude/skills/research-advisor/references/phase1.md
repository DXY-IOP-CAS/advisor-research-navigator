...
- 阶段划分依据：**教授的真实学术履历阶段**（博士 → 博后 → 副研 → 独立），不是按年份区间
- 当前后两阶段研究方向差异大时，叙事需要说明"为什么转向"（如：从碰撞物理转向阿秒光学）
- "禁写" `代表性论文`、`以下见完整列表`、`如下图` 等字样
- 论文表格由脚本生成，AI 不得修改表格行数或内容。只写表格前的叙事
- 脚本默认按年份分组，AI 必须在叙事中重写 `### 4.x` 标题为真实阶段名
  - ❌ `### 4.5 2025–2029`（脚本默认，AI 需覆盖）
  - ✅ `### 4.5 独立科研阶段（2024–至今，中科院物理所特聘研究员）`
- **铁规**：每个 `### 4.x` 标题中**必须含中文阶段描述字**（"博士"、"博士后"、"研究员"、"独立"、"教授"等）。纯数字/纯年份的阶段名直接判失败。
- AI 必须用 Edit 逐条替换 `### 4.x` 标题，连同说明性叙事一起更新。不能跳过任何阶段不改。
