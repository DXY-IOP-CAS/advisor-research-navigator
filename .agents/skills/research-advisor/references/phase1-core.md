...
按以下顺序用当前可用的网页读取和检索工具搜索（不要漏步骤、也不要乱序）。工具可以不同，证据顺序不能乱：
...
如果 `next_actions` 要求补官网英文名或当前官网邮箱域，先核到官方 URL，再用受控脚本更新当前 active state：
...
画像的读者默认只有高等数学、线性代数和普通物理基础，可能没有稳定掌握量子力学、光学、谱学和具体前沿方向。义务：

- 每个专业术语首次出现时用 1 句话解释（例："高次谐波产生（HHG）——用强激光打气体/固体，产生极紫外脉冲"）
- 机构缩写和领域缩写首次出现时写全称（例："日本理化学研究所（RIKEN）"、"原子、分子与光学物理（Atomic, Molecular and Optical Physics, AMO）"）
- 每篇论文叙事回答"为什么要做这个"（背景）和"这发现了什么"（发现），不只是报标题
- 分层递进：先研究背景和技术原理，再讲具体发现
- 开头必须说明画像解决什么问题、为什么先看画像、读完应获得什么能力；不要假设读者已经看过导读或项目规则。
...
1 段总体概述 + 3-4 个具体方向（中英文关键词）。客观描述，不做评价。段落过长时拆成短段或表格，避免把多个并列关系压成一整段。
