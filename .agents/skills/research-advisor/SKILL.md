...
用户和 AI 逐文档、逐小节审查成品时，执行“讨论即沉淀”。不要只在聊天里达成共识，也不要只改当前导师成品。每次形成可复用结论后，先分流：设计原因进 `docs/计划书.md`；当前状态和下一步进 `docs/上下文交接.md`；阶段执行细节进本 skill 的 `references/`；可复用骨架进 `assets/templates/`；可机械检查的底线进 `scripts/` 和 tests；只影响单个导师表达的问题先留在该导师成品中。讨论顺序必须先定整份文档职责和大纲，再逐节审查正文逻辑、图文配合和出口能力，最后回看整篇及 `00-04` 整体衔接。

可视化服务理解，不服务装饰。不要把所有内容强行画成 Mermaid 流程图；领域定位优先层级树或定位表，论文路线优先矩阵表，学习向导优先课程-能力-论文桥接表，只有真实流程或平台链路才优先用小型 Mermaid。每个视觉构件必须先回答一个读者问题，并在附近说明读法和学生看完应能复述的判断；无分支、无边标签、4 个以上节点的线性 Mermaid 链条通常只是关键词串联，不能作为有效理解图。
...
| 确定性 smoke | run `python .agents/skills/research-advisor/scripts/verify_phase_docs.py --prof-dir "<prof_dir>"` |
| DOI 来源元数据 smoke | run `python .agents/skills/research-advisor/scripts/verify_source_metadata.py --prof-dir "<prof_dir>"` when network is available |
