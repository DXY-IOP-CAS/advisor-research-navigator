...
严格按 `END_TO_END_TEST.md` 的格式，只给 subagent 以下内容：
...
- `END_TO_END_TEST.md` 示例使用 `中科院物理研究所`
...
- 同步更新 `END_TO_END_TEST.md`，避免测试输入和 canonical 路径冲突。
...
但它也暴露了两个应优先修的工程一致性问题：

1. 机构路径 canonicalization 缺失。
2. `render_profile.py --prof-dir` 默认输出路径与文档承诺不一致。

下一步建议先修 H2，因为它是明确代码 bug；再修 H1，因为它会影响 output 目录稳定性。
