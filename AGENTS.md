...
Subagent 是本项目的质量门，不是把主 agent 的想法换个声音复述。任务合同里的“隔离审查”要先判断是否需要 subagent、审查轴是什么、结果回灌到哪里。高风险任务完成主 agent 自审后，应主动开只读 subagent 做外部回放，尤其是 `00-04` 成品、`D00-D04` 标准、`S4` 可视化、`03_论文路线.md`、`04_学习向导.md`、长上下文后的重构和用户已经指出质量不可信的文件。

本项目已授权在高风险开发和审查中主动使用 subagent；这条项目入口规则就是后续短指令的默认授权，不需要等用户在每个窗口再次点名。新窗口只要任务合同显示有独立审查轴或明确可并行侧线，就应开相应 subagent。主 agent 保留层级锁定、最终决策和当前层级编辑；subagent 负责并行读材料、隔离审查、验证或小白回放。

每次非小任务的 `隔离审查` 不能留空，只能写三种结论之一：

```text
不开：只改错字、路径、标题、机械格式；或只是单段措辞调整，且不改变证据、主线、图文和学生输出；或本次已经是单轴只读审查，没有并行分轴价值。
需要：命中高风险触发器，且已有冻结稿可只读审查，或当前已有明确可并行侧线，例如探索、证据预审、图文片段审查、验证长日志归纳。
延后：当前没有冻结稿，也没有明确可并行侧线，但任务完成后会产生高风险稿件；必须写明冻结点、审查轴和回灌位置。
```

默认使用六类常用 subagent 角色；它们归入探索、审查、验证、工作四种工作模式：

| 类型 | 何时使用 | 只交付什么 |
|:---|:---|:---|
| 探索型 | 要读很多文件、论文、来源或旧规范，但只需少量判断 | 路径、证据、冲突点和可用结论 |
| 小白读者回放 | 判断学生是否真的能沿文档走到出口 | 实际阅读路线、卡住位置、能否完成学生输出 |
| 证据审查 | 身份、论文归属、领域定位、目标论文选择有风险 | 判断是否被来源支撑、哪里需复核 |
| 图文审查 | 出现图、表、Mermaid、SVG、路线图或证据卡 | 图是否降低理解负担，是否应删掉或降级 |
| 验证型 | 测试、渲染、链接或长日志会污染主线程 | 通过项、失败项、关键日志和下一步 |
| 工作型 | disjoint 文件或模块的机械改动 | 修改范围、变更摘要和验证结果 |

默认需要 subagent 的触发器：

- `00-04` 成品或 `D00-D04` 标准完成一轮实质修订。
- `03_论文路线.md`、`04_学习向导.md` 或对应标准。
- `S4` 可视化、图表密集段落、Mermaid、SVG、路线图或证据卡。
- 身份锁定、论文归属、领域定位、目标论文选择。
- 长上下文、上下文压缩、多次被用户指出偏差，或用户已经指出某文件质量不可信。
- 准备声称“读者路径清楚”“图帮助理解”“学习路线可执行”“论文路线能被小白复述”。

审查轴选择：

- 学生是否能沿文档走到出口：开小白读者回放。
- 判断是否被来源支撑、哪里需复核：开证据审查。
- 图、表、Mermaid、SVG 是否降低理解负担：开图文审查。
- 大量读材料但只需少量判断：开探索型 subagent。
- 长日志、测试、渲染或 smoke 会污染主线程：开验证型 subagent。
- disjoint 文件或模块的机械改动：可开工作型 subagent；不得让多个 subagent 并行改同一文件。

Subagent 输入必须隔离：只给冻结稿、读者身份、预期出口和必要文件路径；不要塞完整聊天记录、作者意图、旧解释或主 agent 的辩护。Subagent 默认只读，不与主 agent 并行修改同一批文件。主 agent 必须把 subagent 的“实际读到”与原设计路线对照，决定删、改、拆、降级或退回上游方法；不能把 subagent 反馈当作可选润色意见。

没有经过合适的 subagent 回放，不要声称“读者路径清楚”“图帮助理解”“学习路线可执行”“论文路线能被小白复述”。如果本次没有使用 subagent，要说明原因，例如任务很小、只改格式、或当前环境无法并行审查。
