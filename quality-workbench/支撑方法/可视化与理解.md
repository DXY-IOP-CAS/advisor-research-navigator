...
flowchart TB
  ...
  classDef check fill:#f2e7c9,stroke:#3a342c,stroke-width:2px,stroke-dasharray:5 4,color:#2f2a24;
  class A,B,C,D step;
  class E check;
```
...
%%{init: {"theme":"base","themeVariables":{"background":"transparent","primaryColor":"#fffaf0","primaryTextColor":"#2f2a24","primaryBorderColor":"#3a342c","lineColor":"#5f5548","fontFamily":"Microsoft YaHei, Noto Sans CJK SC, sans-serif"}}}%%
flowchart TB
  A["早期训练<br/>只作背景，不直接解释当前方向"] --> B["转向阶段<br/>观察论文主题是否变化"]
  B --> C["当前机构<br/>当前方向主证据，后续 02-04 优先消费"]
  classDef background fill:#f8f3e8,stroke:#5f5548,stroke-width:2px,color:#2f2a24;
  classDef current fill:#d8eac8,stroke:#3a342c,stroke-width:3px,color:#2f2a24;
...
<svg width="100%" viewBox="0 0 760 320" role="img" aria-label="领域范围定位图示意" style="font-family:'Microsoft YaHei','Noto Sans CJK SC',sans-serif">
  <defs>
...
flowchart TB
  P["共同问题：如何解释某类现象"]
  P --> A["底座论文<br/>定义对象或平台"]
  P --> B["方法论文<br/>给出测量或计算工具"]
  P --> C["目标论文<br/>回答当前核心问题"]
  P --> D["扩展论文<br/>换材料、条件或尺度"]
  P -.-> E["旁支论文<br/>相关但不进入主线"]
  classDef problem fill:#f2e7c9,stroke:#3a342c,stroke-width:3px,color:#2f2a24;
  classDef role fill:#fffaf0,stroke:#3a342c,stroke-width:2px,color:#2f2a24;
  classDef weak fill:#f8f3e8,stroke:#3a342c,stroke-width:2px,stroke-dasharray:5 4,color:#5f5548;
  class P problem;
  class A,B,C,D role;
  class E weak;
```
...
flowchart TB
  ...
  classDef start fill:#d8eac8,stroke:#3a342c,stroke-width:2px,color:#2f2a24;
  classDef gap fill:#fffaf0,stroke:#3a342c,stroke-width:2px,color:#2f2a24;
  classDef target fill:#f2b6a0,stroke:#3a342c,stroke-width:3px,color:#2f2a24;
  classDef fallback fill:#f2e7c9,stroke:#3a342c,stroke-width:2px,stroke-dasharray:5 4,color:#2f2a24;
  class A start;
  class B,C gap;
  class D,E target;
  class F fallback;
